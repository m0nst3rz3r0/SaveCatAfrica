        ...
        >
          <div className="inline-flex items-center gap-2 bg-terracotta/20 text-terracotta px-4 py-1.5 rounded-full w-fit">
            ...
            <span className="font-inter text-[10px] uppercase tracking-widest font-bold">
              {settings?.heroBadge ?? "Emergency Response Active"}
            </span>
          </div>

          <h1 className="font-inter text-5xl sm:text-6xl md:text-8xl text-white font-bold leading-[1.1] tracking-tight">
            {settings?.heroHeadline ?? "Protecting Domestic"} <br />
            <span className="text-terracotta">
              {settings?.heroHeadlineAccent ?? "Cats Across Africa"}
            </span>
          </h1>

          <p className="font-inter text-lg sm:text-xl text-white/90 max-w-3xl leading-relaxed">
            {settings?.heroSubcopy ??
              "Millions of domestic cats in Africa suffer from neglect, abuse, and lack of veterinary care."}
          </p>

          <div className="flex flex-col sm:flex-row items-center gap-4 mt-4">
            <a
              href="#impact"
              className="w-full sm:w-auto bg-terracotta hover:bg-sunset text-white font-inter px-10 py-5 rounded-lg font-bold uppercase tracking-widest text-sm transition-all shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 text-center"
            >
              DONATE NOW
            </a>
            <a
              href="#mission"
              className="w-full sm:w-auto border-2 border-white/40 hover:border-white text-white font-inter px-10 py-5 rounded-lg font-bold transition-all backdrop-blur-sm hover:bg-white/10 active:scale-95 text-center"
            >
              Learn More
            </a>
          </div>
